...
def menu():
    ...
    while user_input != 'q':
        if user_input == 'a':
            ...
            find_by = input("What property of the movie is that? ") # 'name' or 'director' or 'year'
...
def find_movie(expected, finder):
    found = []
    for movie in movies:
        if finder(movie) == expected:    # (lambda x: x[find_by])(movie) == looking_for
            found.append(movie)
    return found
